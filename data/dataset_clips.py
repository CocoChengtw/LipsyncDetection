...
class ForensicsClips(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return sample, label, video_idx, video_name


class AVLipsClips(Dataset):
    """Dataset class for AVLips"""
    def __init__(
            self,
            frames_per_clip,
            grayscale=False,
        ...
        ds_types = ['0_real', '1_fake']
        for ds_type in ds_types:
            video_paths = os.path.join('./data', 'datasets', 'AVLips', 'cropped_mouths', ds_type)
    ...
    def __getitem__(self, idx):
        ...
        label = 0 if video_idx < self.videos_per_type['0_real'] else 1
...
class DemoClips(Dataset):
    ...
    def __init__(
        ...
        ds_types = ['safe_videos', 'scam_videos']
        
        for ds_type in ds_types:
            video_paths = os.path.join('./data', 'datasets', 'DemoDataset', 'cropped_mouths_orig_gemini_exp4', ds_type)
            videos = sorted(os.listdir(video_paths))
            ...
            for video in videos:
                ...
                # 若影片中的 frame 數少於 50，則直接跳過此影片
                if frame_count < 50:
                    continue
                valid_videos_count += 1
                # 如果 frame 數大於設定的 max_frames_per_video，就取最大值
                num_frames = min(frame_count, max_frames_per_video)
                # num_frames = len(os.listdir(path))
                num_clips = num_frames // frames_per_clip
                self.clips_per_video.append(num_clips)
                self.paths.append(path)
                
            self.videos_per_type[ds_type] = valid_videos_count
    ...
    def __getitem__(self, idx):
        ...
        label = 0 if video_idx < self.videos_per_type['safe_videos'] else 1
        label = torch.tensor(label, dtype=torch.float32)
...
class TestClips(Dataset):
    ...
    def __init__(
        ...
        ds_types = ['0_real', '1_fake']
        for ds_type in ds_types:
            video_paths = os.path.join('./data', 'datasets', 'test', 'cropped_mouths', ds_type)
            videos = sorted(os.listdir(video_paths))
