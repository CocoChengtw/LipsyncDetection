...
def get_model(weights_forgery_path=None, device="cuda:0"):
    ...
    # 解析 device 字串，例如 "cuda:0"
    if isinstance(device, str) and device.startswith("cuda:"):
        try:
            device_index = int(device.split(":")[1])
        except (IndexError, ValueError):
            device_index = 0  # 若解析失敗，預設使用 0
    else:
        device_index = None  # 若不是 cuda 或其他狀況，直接使用預設的 map_location
    ...
    if weights_forgery_path is not None:
        ...
        checkpoint_dict = torch.load(
            ...
            map_location=lambda storage, loc: storage.cuda(device_index) if device_index is not None else storage
        )
